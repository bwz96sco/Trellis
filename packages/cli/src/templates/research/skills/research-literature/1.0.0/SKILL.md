...
3. Read skeleton-first: research question, stated gap, field map, method modules, assumptions, experiments, ablations, limitations, and application scenario.
4. When requested, use `templates/note-template.md` without changing its evidence categories. Anchor material claims to sections, pages, tables, or results.
5. Keep author-stated limitations, observed failures, and analyst inferences separate. For abstract-only access, mark deeper analysis as not assessable.
6. Update only the declared register or note output. Report blocked access or uncertain metadata instead of filling gaps by inference.
...
Stop after the requested paper/register unit is complete, when the question or evidence is insufficient, or when the work would become replacement-method ideation. Paper-local defect diagnosis belongs here; proposing replacement methods belongs to a separately selected ideation package.
