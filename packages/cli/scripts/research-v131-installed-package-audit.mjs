...
const TASK_ROOT = path.join(
  REPO_ROOT,
  ".trellis/tasks/08-12-integrate-install-and-freeze-v1-3-1-subject",
);
...
const T4_COMMIT = "ff7baa58c2a668af8e3e53f7614f814c9a734cc5";
const T4_TREE = "1489191d46f7ccf6d42654063b9352777c72f683";
...
function buildInputAttestation(candidateManifest) {
  if (gitTree(T4_COMMIT) !== T4_TREE || gitTree(A133_COMMIT) !== A133_TREE || gitTree(B133_COMMIT) !== B133_TREE || gitTree(O133_COMMIT) !== O133_TREE) {
...
function stagedDiff(relativePath) {
  return execFileSync(
    "git",
    ["-C", REPO_ROOT, "diff", "--cached", "--binary", "--", relativePath],
    { encoding: null },
  );
}
...
function buildProtectedAudit() {
  ...
  const topology = JSON.parse(
    gitObjectBytes(T4_COMMIT, G0_TOPOLOGY_PATH).toString("utf8"),
  );
  const i1Inventory = topology.stageInventories?.I1;
  if (
    i1Inventory?.count !== 8 ||
    !Array.isArray(i1Inventory.paths) ||
    i1Inventory.paths.length !== 8 ||
    new Set(i1Inventory.paths).size !== 8
  ) {
    throw new Error("Committed G0 I1 inventory is not the exact eight-path set");
  }
  ...
  const allowedPaths = new Set([...i1Inventory.paths, ...inheritedPaths]);
  ...
  const missingI1Paths = i1Inventory.paths.filter((entry) => {
  ...
  if (
    ...
    missingI1Paths.length > 0
  ) {
    throw new Error(
      `I1 worktree scope mismatch: unexpectedDirty=${unexpectedDirtyPaths.join(",")} unexpectedStaged=${unexpectedStagedPaths.join(",")} missing=${missingI1Paths.join(",")}`,
    );
  }
  const worktreeScope = {
    inventorySourceCommit: T4_COMMIT,
    i1PathCount: i1Inventory.count,
    inheritedPathCount: inheritedPaths.length,
    allI1PathsPresentOrProduced: true,
  ...
  const files = baseline.files.map((record) => {
    const stagedBytes = stagedDiff(record.path);
    const actual = {
      path: record.path,
      sha256: sha256(fs.readFileSync(path.join(REPO_ROOT, record.path))),
      gitDiffBinarySha256: diffDigest(record.path),
  ...
  const submodules = baseline.submodules.map((record) => {
    const cwd = path.join(REPO_ROOT, record.path);
    ...
    const indexLine = gitText(["ls-files", "--stage", "--", record.path]).trim();
  ...
  const untrackedBytes = fs.readFileSync(path.join(REPO_ROOT, untracked.path));
  const untrackedRecord = { path: untracked.path, tracked: gitText(["ls-files", "--", untracked.path]).trim() !== "", sha256: sha256(untrackedBytes) };
...
export function runInstalledPackageAudit(mode = "verify") {
  ...
  try {
    ...
    const installEvidence = {
      schemaVersion: 1,
      recordKind: "t5-external-install-evidence",
      stage: "T5",
      commitBoundary: "I1",
      tarballs: {
    ...
    writeOrVerify(
      { input, tarballs: { schemaVersion: 1, recordKind: "t5-package-tarball-inventory", stage: "T5", commitBoundary: "I1", ...packageInventory, verdict: "pass" }, install: installEvidence, protected: protectedAudit },
      mode,
    );
...
if (direct) {
  ...
  process.stdout.write(`T5 installed-package audit ${mode} passed\n`);
}
