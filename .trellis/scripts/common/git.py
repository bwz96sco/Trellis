...
def resolve_default_branch(repo_root: Path) -> str | None:
    """Resolve the local ``refs/remotes/origin/HEAD`` target.

    Task creation is a local operation. It must not contact ``origin`` merely
    to choose metadata; when the local symbolic ref is unavailable, callers
    fall back to the checked-out branch and explain ``--base-branch``.
    """
