...
Candidate-pre-commit integration evidence records its authenticated preparation head. Retained verification must reconstruct evidence from that frozen preparation identity, while allowing repository `HEAD` to equal or descend from it. `--verify` must not treat unrelated ambient worktree or index paths as historical evidence: it reconstructs stored candidate-scope fields with empty unexpected-path arrays, while still rejecting protected-path staging, protected-file digest drift, protected-gitlink drift, unrelated heads, and replacement of historical preparation identity with mutable current-`HEAD` bytes. Every verify-mode package-tree comparison uses the committed range from authenticated governance to authenticated candidate commit; only `--write` may compare governance against candidate worktree bytes. Stored lane counts remain exact historical observations, while current partition tests assert discovery/union equality and dedicated ownership without requiring current counts to equal frozen observations. This keeps normal pre-commit verification independent of the new commit it is validating.

During evidence creation with `--write`, candidate-scope checks remain strict. An authorized successor phase may allow only its exact declared runtime inventory without adding it to frozen candidate inventory; the allowance must bind to authenticated phase/base topology and reject every other unexpected path. Retained T0A allowance remains bound to its existing authenticated correction descendant. Reordered M0-correction bootstrap remains fixed to T0A commit `4b1bb3cd34e574888025187247c2288f3be5195d`: exactly one descendant commit whose changed inventory is exact three-file correction may allow only exact six governance files under `.trellis/tasks/08-21-govern-t6-mal1-attempt-4-m0-correction/`; exactly two descendant commits whose aggregate changed inventory is that correction plus those six governance files may allow only exact three M0 reviewer-definition files. No generic task-directory or mutable-task-derived scope is permitted. Until a matching S3 freeze exists, integration tests authenticate exact current candidate task state. Once S3 for current evidence execution is committed, they authenticate task lifecycle bytes from that freeze's committed I3 subject instead of requiring mutable successor task state to remain pre-closure.
