...
Candidate-pre-commit integration evidence records its authenticated preparation head. Retained verification must reconstruct evidence from that frozen preparation identity, while allowing the repository `HEAD` to equal or descend from it. During an authorized successor phase, verification may allow that phase's exact declared artifact path in the worktree and index without adding it to the frozen candidate inventory; every other unexpected path remains rejected. Until a matching S3 freeze exists, integration tests authenticate the exact current candidate task state. Once S3 for the current evidence execution is committed, they authenticate task lifecycle bytes from that freeze's committed I3 subject instead of requiring mutable successor task state to remain pre-closure. Verification must reject unrelated heads and must not replace historical preparation identity with mutable current-`HEAD` bytes.
