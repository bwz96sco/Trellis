...
Candidate-pre-commit integration evidence records its authenticated preparation head. Retained verification must reconstruct evidence from that frozen preparation identity, while allowing the repository `HEAD` to equal or descend from it. During an authorized successor phase, verification may allow that phase's exact declared runtime inventory in the worktree and index without adding it to the frozen candidate inventory; the runtime allowance must be bound to the authenticated phase/base topology, must not change serialized retained evidence, and must reject every other unexpected path. The retained T0A allowance remains bound to its existing authenticated correction descendant. The reordered M0-correction bootstrap is separately fixed to T0A commit `4b1bb3cd34e574888025187247c2288f3be5195d`: exactly one descendant commit whose changed inventory is the exact three-file correction may allow only the exact six governance files under `.trellis/tasks/08-21-govern-t6-mal1-attempt-4-m0-correction/`; exactly two descendant commits whose aggregate changed inventory is that correction plus those six governance files may allow only the exact three M0 reviewer-definition files. No generic task-directory or mutable-task-derived scope is permitted. Until a matching S3 freeze exists, integration tests authenticate the exact current candidate task state. Once S3 for the current evidence execution is committed, they authenticate task lifecycle bytes from that freeze's committed I3 subject instead of requiring mutable successor task state to remain pre-closure. Verification must reject unrelated heads and must not replace historical preparation identity with mutable current-`HEAD` bytes.
