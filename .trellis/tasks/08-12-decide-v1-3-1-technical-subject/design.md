# T7 — Decide exact v1.3.1 technical subject design

## Boundary

Authenticate the exact frozen subject and machine assurance, then record exactly one accountable operator technical outcome and stop. The stage consumes immutable predecessor Git objects and writes only its exact inventory. Mutable worktree candidate bytes are never semantic authority.
...
exact committed predecessor inputs
  ...
  -> accountable-technical-operator-unassigned
...
The accountable actor for this stage must match `accountable-technical-operator-unassigned` and satisfy the T0 identity-separation matrix. T6 cannot share an identity or scratch space with T0-T5. T7 must be a separately instructed accountable operator.
