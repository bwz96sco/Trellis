...
Presence of `research-quest.yaml` alone does not establish governance. Treat this
literature workspace as quest-governed only when the user explicitly placed the
campaign under the named quest, or the quest links this register or notes through
`next_action.expected_artifact` or `authoritative_artifacts`.

When that link exists and the register or notes changed, prepare one event, tell
the user to invoke `$research-quest-admin` explicitly, then stop; this skill never writes quest state.
When the link is absent or ambiguous, do not prepare a quest
event or admin handoff.
