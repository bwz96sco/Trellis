...
class HarnessFixture(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.research = self.repo / ".trellis" / "tasks" / "c9-test" / "research"
...
class StaticEvidenceTests(HarnessFixture):
    ...
    def test_forward_identities_are_current_c9_and_production_resolved(self) -> None:
    ...
    def test_schema_has_strict_append_record_variants(self) -> None:
        ...
        self.assertEqual(
            schema["$id"], "https://trellis.local/schemas/c9-run-record.schema.json"
        )
        self.assertEqual(schema["title"], "C9 append-only evaluation ledger record")
        self.assertEqual(
...
class WorkspaceTests(HarnessFixture):
    ...
    def test_arms_materialize_only_their_own_frozen_method(self) -> None:
        ...
        self.assertTrue(
            ...
            .startswith("# C9 isolated single-turn execution\n")
        )
        self.assertEqual(
...
class LedgerAndRunnerIntegrationTests(HarnessFixture):
    def test_fake_executor_accounts_reservation_before_result(self) -> None:
        ...
        self.assertEqual(records[1]["process"]["command"][0], "claude")
...
class SummaryAndProofTests(HarnessFixture):
    def test_empty_ledger_summary_and_decision_are_explicitly_pending(self) -> None:
        ...
        self.assertTrue(decision.startswith("# C9 first-party migration decision\n"))
