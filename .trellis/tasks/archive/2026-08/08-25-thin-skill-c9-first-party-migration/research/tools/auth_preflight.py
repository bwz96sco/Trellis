#!/usr/bin/env python3
"""Record or verify the C9 first-party Claude authentication route without a model call."""
...
from claude_runner import build_first_party_environment
...
class AuthPreflightError(ValueError):
    """Raised when authentication routing or its evidence violates the C9 contract."""
...
def build_auth_artifact(raw_status: bytes, *, checked_at: str) -> dict[str, Any]:
...
def run_auth_preflight(
    ...
    *,
    ...
    artifact = build_auth_artifact(raw_status, checked_at=checked_at or utc_now())
