#!/usr/bin/env python3
"""Forward-only C8 evaluation preparation, ledger, and deterministic proof tools.
...
AUTHORIZATION_REF = "c8-claude-sonnet-5-18-plus-6-infrastructure-retries"
CASE_IDS = (
    "literature-01",
    "literature-02",
    "literature-03",
...
class HarnessError(ValueError):
    """Raised when frozen evidence or append-only state violates the C8 contract."""
...
def load_plan(research_root: Path = RESEARCH_ROOT) -> dict[str, Any]:
    ...
    if plan.get("schemaVersion") != 1 or plan.get("evaluationId") != "thin-skill-c8-full-migration":
        raise HarnessError("evaluation-plan.json is not the forward C8 plan")
    if tuple(plan.get("cases", ())) != CASE_IDS:
        raise HarnessError("evaluation plan must list the exact nine C8 cases once in order")
...
def _validate_reservation_shape(record: Mapping[str, Any]) -> None:
    ...
    if record["authorizationRef"] != AUTHORIZATION_REF:
        raise HarnessError("reservation authorizationRef is not the finite C8 authorization")
...
def _append_record_object(record: Mapping[str, Any], research_root: Path) -> str:
    with tempfile.NamedTemporaryFile(
        mode="wb", suffix=".json", prefix="c8-record-", delete=False, dir=research_root
    ) as stream:
...
def validate_deterministic_proof(
    ...
    *,
    ...
    if proof.get("schemaVersion") != 1 or proof.get("proofId") != "thin-skill-c8-deterministic-proof-v1" or proof.get("status") != "passed":
...
def generate_deterministic_proof(
    ...
    *,
    ...
    proof = {
        "schemaVersion": 1,
        "proofId": "thin-skill-c8-deterministic-proof-v1",
        "evaluationId": plan["evaluationId"],
        "generatedAt": utc_now(),
        "status": "passed",
        "sourceBaseline": {
