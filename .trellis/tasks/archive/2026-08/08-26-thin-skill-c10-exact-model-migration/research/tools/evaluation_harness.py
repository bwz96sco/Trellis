#!/usr/bin/env python3
"""Forward-only C10 evaluation preparation, ledger, and deterministic proof tools.
...
EVALUATION_ID = "thin-skill-c10-exact-model-migration"
PROOF_ID = "thin-skill-c10-deterministic-proof-v1"
AUTHORIZATION_REF = "c10-first-party-claude-sonnet-5-18-plus-6-infrastructure-retries"
CASE_IDS = (
    "literature-01",
    "literature-02",
    "literature-03",
...
class HarnessError(ValueError):
    """Raised when frozen evidence or append-only state violates the C10 contract."""
...
def load_plan(research_root: Path = RESEARCH_ROOT) -> dict[str, Any]:
    ...
    if plan.get("schemaVersion") != 1 or plan.get("evaluationId") != EVALUATION_ID:
        raise HarnessError("evaluation-plan.json is not the forward C10 plan")
    if tuple(plan.get("cases", ())) != CASE_IDS:
        raise HarnessError("evaluation plan must list the exact nine C10 cases once in order")
    ...
    expected_runner_contract = {
        "addDirForbidden": True,
        "advisorDisabled": True,
        "binary": DIRECT_CLAUDE_EXECUTABLE,
        "expectedApiProvider": EXPECTED_API_PROVIDER,
        "expectedVersion": EXPECTED_CLAUDE_VERSION,
        ...
        "promptSuggestionsDisabled": True,
        "workingDirectory": "outputs/<logical-run-id>/workspace",
    }
    if plan.get("runnerContract") != expected_runner_contract:
        raise HarnessError("runner contract differs from the exact direct CLI command")

    expected_live_gate = {
        "allApplicableAssertionsMustPass": True,
        "firstPlannedSlotIsExactModelProbe": True,
...
def _predecessor_evidence(research_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or predecessor.get("evaluationId") != "thin-skill-c9-first-party-migration"
        or predecessor.get("status") != "blocked-nonretryable-provider-failure"
    ):
        raise HarnessError("predecessor.json does not bind the exact blocked C9 identity")
    return {
...
def validate_static_evidence(
    ...
    if (
        schema.get("$id") != "https://trellis.local/schemas/c10-run-record.schema.json"
        or schema.get("title") != "C10 append-only evaluation ledger record"
        or schema.get("$defs", {})
        ...
        raise HarnessError("run-record.schema.json does not carry the exact C10 identity")
...
def prepare_run(
    ...
    try:
        ...
        system_parts = [
            "# C10 isolated single-turn execution",
            "",
            method_header,
...
def _validate_reservation_shape(record: Mapping[str, Any]) -> None:
    ...
    if record["authorizationRef"] != AUTHORIZATION_REF:
        raise HarnessError("reservation authorizationRef is not the finite C10 authorization")
...
def _append_record_object(record: Mapping[str, Any], research_root: Path) -> str:
    with tempfile.NamedTemporaryFile(
        mode="wb", suffix=".json", prefix="c10-record-", delete=False, dir=research_root
    ) as stream:
...
def validate_deterministic_proof(
    ...
    *,
    ...
    for check in checks:
        ...
        if command[0] == DIRECT_CLAUDE_EXECUTABLE or Path(command[0]).name.lower() in {
            "claude",
            "codex",
            "gemini",
        }:
...
def write_summary_and_decision(research_root: Path = RESEARCH_ROOT) -> tuple[Path, Path]:
    ...
    if gate["status"] == "passed":
        ...
        "blocked-nonretryable-provider-failure",
        "failed-zero-tolerance",
        "failed-quality",
    }:
        decision = "BLOCKED"
        reason = (
            f"live/full-migration gate terminated as {gate['status']}; "
