# T1 — Conform Core runtime to evaluation-contract v1.3.1 design

## Boundary

Implement a version-explicit Core runtime path that enforces the exact seven accepted v1.3.1 leaves and deterministic report-v2 semantics without reinterpreting historical v1.3.0 identities. The stage consumes immutable predecessor Git objects and writes only its exact inventory. Mutable worktree candidate bytes are never semantic authority.
...
exact committed predecessor inputs
  ...
  -> core-v131-implementer-unassigned
...
The accountable actor for this stage must match `core-v131-implementer-unassigned` and satisfy the T0 identity-separation matrix. T6 cannot share an identity or scratch space with T0-T5. T7 must be a separately instructed accountable operator.
