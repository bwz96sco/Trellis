...
## Additive overlay — evaluation-contract v1.3.1 successor (2026-08-08)

CS6-1 A11 `3534529a36a10ea8015a51f71a93e2b78300a563` independently authenticated v1.3.0 but proved four accepted-contract defects: no closed report-v2 schema, no rule-specific validator fact schemas/predicates, label-only global differential mutations without applicability predicates, and no authoritative Procedure/capability-to-artifact-family mapping. Its disposition is `contract-defect`; CS6-2 through CS6-8 remain blocked and are not rebound.

The additive contract-only campaign `.trellis/tasks/08-08-correct-evaluation-contract-v1-3-1-semantic-defects/` contains exactly Author, MAL-1 Assurance, and Operator Decision children. It preserves all v1.3.0 and A11 bytes, permits semantic changes only for `CS6-1-CONTRACT-001` through `004` plus the finite G131-frozen contract-identity, exact member-reference, provenance-class, and finding-bound `recordRef` transitions, and stops after a separate operator decision. Manifest and semantic-target digest recomputation remains non-leaf author evidence. Runtime, CLI, Procedure packages, harnesses, live selection, activation, archive, release, publication, and push remain unauthorized.

G131 is limited to the exact 36-path governance set. Future boundary inventories are exact: A131-0 2, A131-1 15, B131-0 2, B131-1 11, O131-0 2, and O131-1 1. Any authenticated B131-1 pass or fail may reach a separately instructed operator; acceptance requires pass, while reject/stop may follow either verdict. The seven successor leaves preserve all frozen populations, including 11 immutable lifecycle artifact-family enum values distinct from 4 closure families, embed global fixtures in the differential leaf, and reject any pointer outside the descendant-closed direct regions and finite exact-guarded propagation domains as a fifth change.
