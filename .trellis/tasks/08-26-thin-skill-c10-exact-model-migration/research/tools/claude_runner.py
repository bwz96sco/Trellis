#!/usr/bin/env python3
"""Exact, single-turn Claude CLI adapter for the C10 live evaluation.
...
EXPECTED_API_PROVIDER = "firstParty"
EXPECTED_MODEL = "claude-sonnet-5"
DIRECT_CLAUDE_EXECUTABLE = "/Users/zhangbowen/.local/share/claude/versions/2.1.228"
EXPECTED_CLAUDE_VERSION = "2.1.228 (Claude Code)"
...
def build_first_party_environment(
    ...
    """Copy an environment, remove parent routing state, and force C10 controls."""
...
def normalize_usable_result(
    ...
    *,
    ...
    if not isinstance(resolved_usage, dict):
        raise RunnerValidationError("resolved model usage must be an object")
    if resolved_usage.get("provider") != EXPECTED_API_PROVIDER:
        raise RunnerValidationError(
            f"provider substitution rejected: expected {EXPECTED_API_PROVIDER}"
        )
